...
void print_inode(struct ext2_inode *inode) {
  ...
  printf("[current] type: %c size: %d links: %d blocks: %d\n",
         get_type_inode(inode), inode->i_size, inode->i_links_count,
         inode->i_blocks);
...
struct ext2_inode *path_walk(char *path, struct ext2_dir_entry_2 **dir_entry) {
  ...
  struct ext2_dir_entry_2 *last =
      get_next_dir_entry(cur, path_array[sections_count - 1]);
  *dir_entry = last;
...
struct ext2_dir_entry_2 *get_next_dir_entry(struct ext2_inode *cur_dir,
                                            char *name) {
